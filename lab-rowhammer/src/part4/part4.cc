...
#define HAMMER_ACCESSES 5000000
...
uint64_t hammer_addresses(uint64_t vict, uint64_t attA, uint64_t attB, uint64_t hp_base)
{
    ...
    clflush((void *)attA);
    clflush((void *)attB);
    volatile char *atkA = (volatile char *)attA;
    volatile char *atkB = (volatile char *)attB;

    // hammer part
    for (int i = 0; i < HAMMER_ACCESSES; i++)
    {
        *atkA;
        *atkB;
    ...
    // probe
    uint64_t vict_base = vict & ~((uint64_t)ROW_STRIDE - 1);
    for (uint64_t i = 0; i < ROW_STRIDE; i++)
    {
        uint64_t val = *((volatile uint8_t *)(vict_base + i));
        if (val != VIC_DATA)
        {
            foundFlips++;
        }
